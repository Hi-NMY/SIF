...
public class FragmentIb extends BaseFragment implements View.OnClickListener {
    private View view;
    private RecyclerView ib_recyclerView;
    private InterestingBlock interestingBlock;
    private GridLayoutManager gridLayoutManager;
    private IbClass ibClass;
    ...
    private InitIb initIb;
    private FreshFollowNum freshFollowNum;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        intentFilter.addAction("searchIbZong");
        searchBroadCast = new SearchBroadCast();
        localBroadcastManager.registerReceiver(searchBroadCast,intentFilter);
        IntentFilter intentFilter1 = new IntentFilter();
        intentFilter1.addAction("initIb");
        initIb = new InitIb();
        localBroadcastManager.registerReceiver(initIb,intentFilter1);

        freshFollowNum = new FreshFollowNum();
        BroadcastRec.obtainRecriver(MyApplication.getContext(),"trueFollow",freshFollowNum);
    ...
    public void initIbClass(int i){
        if (i == 1){
            ibClass = interestingBlock.sendIb(1);
        }else {
            ibClass = interestingBlock.sendIb(2);
        }
    }
    ...
    private void initFrushListener(SmartRefreshLayout m) {
        ...
        m.setOnLoadMoreListener(new OnLoadMoreListener() {
            ...
            public void onLoadMore(@NonNull RefreshLayout refreshLayout) {
                ibClass.myHandler(moreHander);
                ibClass.sendIbMessage(ibClassList.ibId);
            }
        });
    }

    public void searchIb(int i,String name){
        if (i != -1 && name != null && !name.equals("")){
            ibClass.myHandler(searchHanlder);
            ibClass.searchIbMore(i,name);
    ...
    private Handler moreHander = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            ibClassList.addMoreIbList(ibClass.interestingBlockClasses);
        }
    };
    ...
    private Handler nullHanlder = new Handler(){
        ...
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            initIbMore(ibClass.interestingBlockClasses);
            mIbRefresh.finishRefresh();
        }
    };

    public Handler searchHanlder = new Handler(){
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            initIbMore(ibClass.interestingBlockClasses);
    ...
    class InitIb extends BroadcastReceiver{
        @Override
        public void onReceive(Context context, Intent intent) {
            initIbMore(ibClass.interestingBlockClasses);
        }
    ...
    class FreshFollowNum extends BroadcastReceiver{
        @Override
        public void onReceive(Context context, Intent intent) {
            ibClassList.freshFollowNum(intent.getIntExtra("textone",0));
        }
    }
}
